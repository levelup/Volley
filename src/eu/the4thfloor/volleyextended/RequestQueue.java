...
public class RequestQueue {
   ...
   */
  public RequestQueue(final Cache cache, final Network network, final int threadPoolSize, final ResponseDelivery delivery) {
   ...
   *          Number of network dispatcher threads to create
   */
  public RequestQueue(final Cache cache, final Network network, final int threadPoolSize) {
  ...
  public void start() {
    ...
    for (int i = 0; i < this.mDispatchers.length; i++) {
      final NetworkDispatcher networkDispatcher = new NetworkDispatcher(this.mNetworkQueue, this.mNetwork, this.mCache, this.mDelivery);
  ...
  void finish(final Request request) {
    ...
    if (request.shouldCache()) {
      synchronized (this.mWaitingRequests) {
        final String cacheKey = request.getCacheKey();
        final Queue<Request> waitingRequests = this.mWaitingRequests.remove(cacheKey);
