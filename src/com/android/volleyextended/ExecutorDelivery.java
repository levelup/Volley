...
public class ExecutorDelivery implements ResponseDelivery {
   ...
   */
  public ExecutorDelivery(final Executor executor) {
  ...
  public void postError(final Request<?> request, final VolleyError error) {
    ...
    final Response<?> response = Response.error(error);
  ...
  private class ResponseDeliveryRunnable implements Runnable {
    ...
    public ResponseDeliveryRunnable(final Request request, final Response response, final Runnable runnable) {
    ...
    public void run() {
      ...
      if (this.mResponse.isSuccess()) {
        this.mRequest.deliverResponse(this.mResponse.result, this.mResponse.hasChanged);
      } else {

        Response<?> cachedResponse = null;
        Object cachedData = null;

        final Cache.Entry cacheEntry = this.mRequest.getCacheEntry();
        if (cacheEntry != null) {
          cachedResponse = this.mRequest.parseNetworkResponse(new NetworkResponse(cacheEntry.data, cacheEntry.responseHeaders, true));
          if (cachedResponse != null) {
            cachedData = cachedResponse.result;
          }
        }

        this.mRequest.deliverError(this.mResponse.error, cachedData);
      }
